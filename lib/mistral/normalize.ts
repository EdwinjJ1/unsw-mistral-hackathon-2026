...
/*
 * Track A owns the canonical node/edge ID helpers. Conflict edges add the
 * contract-required endpoint ordering before delegating to edgeId().
 */
import type { DeltaDraft, DeltaNodeDraft } from "./models";
...
export function makeEdgeId(
  ...
  if (type === "CONFLICTS_WITH") {
    ...
    return edgeId(left!, type, right!);
  }
  return edgeId(from, type, to);
}
...
export function normalizeDeltaDraft(
  ...
  for (const nodeDraft of draft.nodes) {
    ...
    const id = existing?.id ?? nodeId(nodeDraft.type, nodeDraft.label);
    plans.push({
      draft: nodeDraft,
      id,
      ...(existing ? { existing } : {}),
    ...
    referenceMap.set(nodeId(nodeDraft.type, nodeDraft.label), id);
  ...
  for (const node of contextNodes) {
    ...
    referenceMap.set(nodeId(node.type, node.label), node.id);
  }
