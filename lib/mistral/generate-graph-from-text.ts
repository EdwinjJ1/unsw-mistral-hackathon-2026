...
import type { Delta, EdgeType, SourceRef } from "../types.js";
import type { DeltaDraft, DeltaEdgeDraft, DeltaNodeDraft } from "./models.js";
import { makeNodeId, normalizeDeltaDraft } from "./normalize.js";
...
function fallbackDocumentDraft(text: string): DeltaDraft {
  ...
  const addNode = (draft: DeltaNodeDraft): string => {
    const id = draft.id ?? makeNodeId(draft.type, draft.label);
