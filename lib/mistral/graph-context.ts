...
function compareAssignments(left: AssignmentFact, right: AssignmentFact): number {
  const leftBlocked =
    left.blockers.length > 0 ||
    left.dependencies.some((dependency) => dependency.target.status === "blocked");
  const rightBlocked =
    right.blockers.length > 0 ||
    right.dependencies.some((dependency) => dependency.target.status === "blocked");
  const actionableOrder = Number(leftBlocked) - Number(rightBlocked);
  const leftDate = left.task.dueDate ?? "9999-12-31";
  const rightDate = right.task.dueDate ?? "9999-12-31";
  return (
    actionableOrder ||
    leftDate.localeCompare(rightDate) ||
    left.task.label.localeCompare(right.task.label)
  );
}

function imperativeInstruction(value: string): string | undefined {
  const instruction = value.replace(/[.!]+$/g, "").replace(/\s+/g, " ").trim();
  if (!/^(confirm|review|approve|provide|complete|send|share|verify|clarify|resolve|deliver)\b/i.test(instruction)) {
    return undefined;
  }
  if (/^confirm\s+(?!the\b)/i.test(instruction)) {
    return instruction.replace(/^confirm\s+/i, "Confirm the ");
  }
  return instruction;
}

function dependencyInstruction(
  target: GraphNode,
  edge: GraphEdge,
): string | undefined {
  if (target.type === "Task") {
    const taskAction = imperativeInstruction(target.label);
    if (taskAction) return taskAction;
  }
  if (!edge.note || edge.note.length > 140) return undefined;
  return imperativeInstruction(edge.note);
}
  ...
  const facts = ownedTasks.map((task): AssignmentFact => {
    ...
    const dependencies = graph.edges
      ...
      .map((edge) => {
        ...
        const instruction = dependencyInstruction(target, edge);
        return {
          target,
          ...(contact ? { contact } : {}),
